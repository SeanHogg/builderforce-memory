...
- **`mambacodejs` source checkout now redundant.** `c:\code\agentic\mambacodejs` was subtree-merged into `packages/memory-core` (history preserved here). The standalone checkout and its `SeanHogg/mambacodejs` GitHub repo can be archived/removed once the new scope is published and consumers are cut over.
