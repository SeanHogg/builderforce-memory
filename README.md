...
- **First npm publish blocked on the `@builderforce` scope / NPM_TOKEN.** The `v2026.6.18` release commit + tag are pushed and CI's Install/Build/Test pass, but the Publish step fails with `npm error 404 Not Found - PUT .../@builderforce/memory-engine`. The `@builderforce` scope has zero packages and its npm org endpoint 404s — so either the org doesn't exist or the `NPM_TOKEN` repo secret (set 2026-03-21) lacks write access to that scope (npm masks an unauthorized publish as 404). To unblock: create/own the `@builderforce` org on npmjs.com, generate a fresh Automation token with read+write to the `@builderforce` scope, update the `NPM_TOKEN` repo secret, then re-run the failed `release.yml` run (or re-push the tag — `2026.6.18` was never actually published, so no version collision). Closing this finally lands the packages on npm so the Claude Code memory install no longer needs a local checkout.
...
- **Coverage excludes the WebGPU session layer (`src/session/**`) and barrels (`src/**/index.ts`).** `pnpm test:coverage` enforces a 100% global threshold, but `collectCoverageFrom` skips `src/session/**` — `MambaSession`, the WGSL/WebGPU `complete`/`completeStream`/`evaluate`/`embed` orchestration, `streaming.ts` sampling, `presets.ts`, and IDB weight `persistence.ts`. These bind to `@builderforce/memory-engine` + a GPU and can't be unit-tested without a WebGPU/`MambaModel` mock harness; the runtime/consumption surface that wraps them is at 100%. Closing this = a GPU-or-mock integration harness that drives `MambaSession` end-to-end (also closes the unverified export→load→recall loop noted above). Until then the engine boundary is covered only by the build + the consumers' own tests.
