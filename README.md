...
- **Mistaken `@seanhogg/memory*` packages need unpublish/deprecate.** Final published names are `@seanhogg/builderforce-memory-engine` / `@seanhogg/builderforce-memory` / `@seanhogg/builderforce-memory-mcp` (the `@builderforce` org doesn't exist on npm, so the scope is `@seanhogg` but the `builderforce-` name prefix is kept). Two earlier same-version (`2026.6.18`) publishes used wrong names: `@builderforce/*` (404'd, never landed) and `@seanhogg/memory{,-engine,-mcp}` (LANDED — published by mistake before the rename). The three `@seanhogg/memory*` packages are live on npm with no consumers and must be cleaned up: `npm unpublish @seanhogg/memory@2026.6.18 --force` (and `-engine`/`-mcp`) within the 72h window, or `npm deprecate "@seanhogg/memory" "Renamed to @seanhogg/builderforce-memory"`. Requires local npm auth (CI token isn't exposed locally); not doable from this commit.
- **Downstream consumers still import `@builderforce/*`.** Any external repo that imported `@builderforce/memory*` must be repointed to `@seanhogg/builderforce-memory*`. In-repo references are all converted; this tracks the out-of-repo rewire (incl. the BuilderForce.ai agent-runtime).
