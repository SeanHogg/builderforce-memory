...
- **Downstream consumers still import `@builderforce/*`.** The packages publish under the `@seanhogg/*` scope (the `@builderforce` org was never created on npm; first-publish 404'd against it, so all three were renamed `@builderforce/memory*` → `@seanhogg/builderforce-memory*` at `v2026.6.18`). Any external repo that already imported `@builderforce/memory*` — and the Claude Code memory install skill's `npx -y @builderforce/memory-mcp` target — must be repointed to `@seanhogg/*`. In-repo references are all converted; this bullet tracks the out-of-repo rewire.
