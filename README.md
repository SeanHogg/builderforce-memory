...
|---|---|---|---|
| [`@seanhogg/memory-engine`](packages/memory-engine) | **Engine** | `@seanhogg/mambacode.js` | WGSL/WebGPU Mamba SSM kernels, model blocks (Mamba1/2/3 + attention), autograd, trainer, BPE tokenizer, quantization. Zero runtime deps. |
| [`@seanhogg/memory`](packages/memory) | **Runtime** | `@seanhogg/ssmjs` | SSM execution, Transformer orchestration (Anthropic/OpenAI/Fetch bridges), online distillation, inference router, sessions, and the persistent `MemoryStore`. Depends on `memory-engine`. |
...
```ts
import { SemanticCache, FetchSemanticCacheBackend, AnthropicBridge } from '@seanhogg/memory';
...
`@seanhogg/memory` consumes `@seanhogg/memory-engine` via the pnpm `workspace:^` protocol; on publish, pnpm rewrites it to a real `^<version>` range.

## Release

All three packages (`memory-engine`, `memory`, `memory-mcp`) version in lockstep (`YYYY.M.D[-beta.N]`).

1. Bump `version` in every `packages/*/package.json` (and root) to the same value, plus the `@seanhogg/memory` range in `memory-mcp`'s `peerDependencies`.
...
1. **Publish the new scope** from this repo (`@seanhogg/memory-engine`, `@seanhogg/memory`).
2. **Deprecate the old names** pointing at the new scope:
   ```bash
   npm deprecate "@seanhogg/mambacode.js" "Moved to @seanhogg/memory-engine"
   npm deprecate "@seanhogg/ssmjs"        "Moved to @seanhogg/memory"
   ```
3. **Update consumers' imports**:
   - `@seanhogg/mambacode.js` → `@seanhogg/memory-engine`
   - `@seanhogg/ssmjs`        → `@seanhogg/memory`
...
- **Downstream consumers still import `@builderforce/*`.** The packages publish under the `@seanhogg/*` scope (the `@builderforce` org was never created on npm; first-publish 404'd against it, so all three were renamed `@builderforce/memory*` → `@seanhogg/memory*` at `v2026.6.18`). Any external repo that already imported `@builderforce/memory*` — and the Claude Code memory install skill's `npx -y @builderforce/memory-mcp` target — must be repointed to `@seanhogg/*`. In-repo references are all converted; this bullet tracks the out-of-repo rewire.
...
- **Coverage excludes the WebGPU session layer (`src/session/**`) and barrels (`src/**/index.ts`).** `pnpm test:coverage` enforces a 100% global threshold, but `collectCoverageFrom` skips `src/session/**` — `MambaSession`, the WGSL/WebGPU `complete`/`completeStream`/`evaluate`/`embed` orchestration, `streaming.ts` sampling, `presets.ts`, and IDB weight `persistence.ts`. These bind to `@seanhogg/memory-engine` + a GPU and can't be unit-tested without a WebGPU/`MambaModel` mock harness; the runtime/consumption surface that wraps them is at 100%. Closing this = a GPU-or-mock integration harness that drives `MambaSession` end-to-end (also closes the unverified export→load→recall loop noted above). Until then the engine boundary is covered only by the build + the consumers' own tests.
