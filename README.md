...
- **builderforce.ai's own agents — memory wiring status (partial; genuine infra blockers).** (1) **On-prem agent hosts** run on a machine with an MCP host → covered by the multi-host installer above. (2) **Cloud Node/container agents** get memory via the existing in-process `ssm-memory-service.ts` (specifier repointed to `@seanhogg/builderforce-memory`); goes live when the `optionalDependencies` entry is added after the 48h `minimumReleaseAge` window (2026-06-20). (3) **Cloud Worker/DO (V2 durable) agents** need the memory packages bundled in-process (no subprocess/npx in Workers) and/or active recall/remember MCP tools surfaced into the embedded runner's custom stream layer (which currently ignores MCP servers) — this is the remaining real work; unblocks active agent-driven recall/remember in cloud runs.
